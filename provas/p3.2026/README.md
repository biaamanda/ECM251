...
| `questao4-modelo/` | Solução da Questão 4 de 2025 (GUI de menus + Checksum + RSA + arquivos + `.jar`) |
...
| `variacoes/afirmacoes/` | Resumo do que cai nas V/F (Q1–Q3) + 4 simulados (36 afirmações) com gabarito separado |
...
| Parte da Q4 | O que pedia |

| I – GUI de menus | 4 menus / 9 subitens, **sem `JOptionPane`**, feedback só na área de mensagens |
| II – Mensagem | ler `msg_original.txt`; calcular CS (Soma e Complemento de 2); salvar `msg_com_cs.txt` |
| III – Chaves | gerar e salvar `chave.publica` e `chave.privada` (RSA) |
| IV – Criptografia | cifrar → `msg_cifrada.txt`; decifrar → `msg_decifrada.txt` |
| V – Geral | mostrar `msg_decifrada.txt` na GUI; `Sair` |
